        ...
        <div class="max-w-5xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">Room: <span class="font-mono font-bold text-indigo-400">{{ $competition->room_code }}</span></p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('judge.end', $competition->room_code) }}" method="POST"
                    ...
                    @csrf
                    <button class="text-xs px-3 py-1 bg-red-800/60 hover:bg-red-700 text-red-300 rounded-lg transition">
                        End Competition
                    </button>
                </form>
                @endif
    ...
    <main class="flex-1 max-w-5xl w-full mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-5">
            ...
            <div class="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                <h2 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Share with contestants</h2>
                <div class="flex gap-2">
                    <input id="join-url" readonly
                           value="{{ url('/join/' . $competition->room_code) }}"
                           class="flex-1 bg-gray-800 rounded-xl px-3 py-2 text-sm text-indigo-300 font-mono truncate border border-gray-700">
                    <button onclick="copyJoinLink()" class="px-4 py-2 bg-indigo-700 hover:bg-indigo-600 text-white rounded-xl text-sm font-semibold transition">
                        Copy
                    </button>
                </div>
                {{-- QR code placeholder --}}
            ...
            <div class="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                ...
                <div id="first-buzzer-area" class="text-center py-6 mb-4
                ...
                <div id="waiting-buzz" class="text-center py-6 mb-4
                ...
                <ul class="space-y-2" id="contestant-list">
                    ...
                    <li id="contestant-{{ $c->id }}"
                        ...
                        <div class="flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full {{ $c->isClaimed() ? 'bg-green-400' : 'bg-gray-600' }}" id="claimed-dot-{{ $c->id }}"></span>
            ...
            <div class="bg-gray-900 rounded-2xl p-5 border border-gray-800">
                ...
                <ol class="space-y-2" id="scoreboard">
                    ...
                    <li class="flex items-center justify-between">
                        ...
                        <span class="font-bold text-white" data-score-id="{{ $c->id }}">{{ $c->score }}</span>
                    </li>
                    @endforeach
                </ol>
            </div>
...
let currentRound = @json($competition->currentRound ? ['id' => $competition->currentRound->id, 'status' => $competition->currentRound->status->value] : null);
...
async function startRound() {
    const res  = await fetch(BASE_URL + '/rounds/start', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
...
async function resetRound() {
    ...
    const res = await fetch(BASE_URL + '/rounds/reset', { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
    if (res.ok) {
        document.getElementById('waiting-buzz').classList.remove('hidden');
        document.getElementById('first-buzzer-area').classList.add('hidden');
        document.getElementById('answer-timer').classList.add('hidden');
...
async function markAnswer(result) {
    ...
    if (res.ok) {
        ...
        if (result === 'correct') {
            document.getElementById('first-buzzer-area').classList.add('hidden');
            document.getElementById('waiting-buzz').classList.remove('hidden');
            document.getElementById('waiting-buzz').querySelector('p').textContent = '✅ Correct! Start next round.';
...
if (window.Echo) {
    window.Echo.channel('competition.' + ROOM_CODE)
        ...
        .listen('.RoundStarted', e => {
            currentRound = { id: e.round_id, status: 'active' };
