    ...
    <div class="w-full max-w-md">
        ...
        <div class="bg-gray-900 rounded-2xl p-5 border border-gray-800 space-y-3">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="font-bold text-xl {{ $loop->first ? 'text-yellow-400' : 'text-white' }}">{{ $contestant->score }}</span>
            </div>
            @endforeach
        </div>

        <div class="mt-8 text-center">
            <a href="{{ route('home') }}" class="inline-block px-6 py-3 bg-indigo-700 hover:bg-indigo-600 text-white font-bold rounded-xl transition">
                New Competition
            </a>
        </div>
    </div>
