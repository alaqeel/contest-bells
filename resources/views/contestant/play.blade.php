    ...
    <div class="flex-1 flex items-center justify-center w-full max-w-xs">
        <button
            ...
            class="w-64 h-64 rounded-full font-black text-4xl shadow-2xl transition-all duration-100 touch-none
                   ...
                   {{ $competition->isEnded() ? 'bg-gray-700 text-gray-500 cursor-not-allowed'
                       : ($competition->currentRound?->status->value === 'active' ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-900/50 cursor-pointer animate-pulse' : 'bg-gray-700 text-gray-500 cursor-not-allowed') }}"
...
async function buzz() {
    ...
    try { document.getElementById('buzz-sound').play(); } catch(e) {}
...
if (window.Echo) {
    ...
    window.Echo.connector.pusher.connection.bind('connecting', () => {
        document.getElementById('connection-dot').className = 'w-2 h-2 rounded-full bg-yellow-400 animate-pulse';
